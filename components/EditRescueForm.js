...
import { StyleSheet, View, Text, TimePickerAndroid, TimePickerAndroidOpenOptions, Picker, TouchableOpacity, Alert  } from 'react-native';
...
export default class EditRescueForm extends Component {
  constructor(props) {
    super(props);
    this.state = {
      location: undefined,
  ...
  componentWillMount() {
    ...
    this.setState({ location: location, boxes: boxes, weight: weight, tags: tags, expirationDate: expiration, listingId: listingId })
  ...
  submit() {
    ...
    Alert.alert(
      ...
      [
        { text: 'Okay', onPress: () => this.props.navigation.navigate('VendorPendingRescues') },
      ],
      { cancelable: false }
    )
  }
    ...
    return (
      <Container style={{ alignSelf: 'center', width: '100%', backgroundColor: '#f6f6f6' }}>
        <Content>
          ...
          >
            <Text style={[styles.buttonText, submitEnabled && styles.buttonAltText]}>UPDATE</Text>
          </Button>
        </Content>
      </Container>
    );
